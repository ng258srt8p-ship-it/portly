...
export interface ExecuteOptions {
  ...
  maxBackoffMs?: number;
  /** Override jitter factor (defaults to config.jitterFactor). */
  jitterFactor?: number;
        ...
        if (shouldRetry(response.status, bodyText) && attempt < maxRetries) {
          ...
          if (options.respectRetryAfter !== false && response.headers?.['retry-after']) {
            ...
            backoffMs = calculateBackoff(
              ...
              options.jitterFactor ?? this.config.jitterFactor,
            );
          }
  ...
  return limiter.executeWithRetry<T>(
    async () => {
      ...
      try {
        ...
        return {
          status: res.status,
          headers: (() => { const h: Record<string, string> = {}; res.headers.forEach((v, k) => { h[k] = v; }); return h; })(),
          body: () => res.text(),
...
export { getGlobalLimiter as nimModelLimiter } from './nimRateLimiter';
