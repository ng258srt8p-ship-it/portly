...
export class CarnivalCruiseLineAdapter extends SourceAdapter {
  ...
  private parseCardText(rawText: string, lines: string[]): SailingRecord | null {
    ...
    let destination = '';
    const destKw = ['Caribbean', 'Bahamas', 'Mexico', 'Europe', 'Mediterranean', 'Alaska', 'Bermuda', 'Panama', 'Galapagos', 'Hawaii', 'Transatlantic'];
    ...
    let departurePort = '';
    const portKw = ['Miami', 'Fort Lauderdale', 'Port Canaveral', 'Tampa', 'Jacksonville', 'Galveston', 'Houston', 'New Orleans', 'Los Angeles', 'Long Beach', 'San Diego', 'Seattle', 'Vancouver', 'Sydney', 'Barcelona', 'Rome', 'Venice', 'Southampton'];
  ...
  private inferRegion(port: string): string {
    ...
    if (p.includes('miami') || p.includes('fort lauderdale') || p.includes('port canaveral') || p.includes('tampa') || p.includes('jacksonville') || p.includes('new orleans')) return 'Florida';
