...
const nimRateLimiter_ts_1 = require("../utils/nimRateLimiter.ts");
const COLORS = {
    reset: '\x1b[0m',
    red: '\x1b[31m',
    green: '\x1b[32m',
...
function statusLog(label) {
    const status = nimRateLimiter_ts_1.defaultRateLimiter.getStatus();
...
async function testConcurrencyLimit() {
    ...
    const promises = Array.from({ length: 10 }, async (_, i) => {
        await nimRateLimiter_ts_1.defaultRateLimiter.acquire();
        const status = nimRateLimiter_ts_1.defaultRateLimiter.getStatus();
        ...
        // Simulate work (100ms)
        await sleep(100);
        nimRateLimiter_ts_1.defaultRateLimiter.release();
...
async function testRpmLimit() {
    ...
    nimRateLimiter_ts_1.defaultRateLimiter.reset();
    await sleep(100);
    const startTime = Date.now();
    let requestCount = 0;
    const timestamps = [];
...
async function testRateLimitWindow() {
    ...
    try {
        ...
        await Promise.race([
            nimRateLimiter_ts_1.defaultRateLimiter.acquire(),
