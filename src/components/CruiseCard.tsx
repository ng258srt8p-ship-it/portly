...
const dealRatingConfig = {
  hot: { icon: 'local_fire_department', label: 'Hot Deal', className: 'badge-hot' },
  great: { icon: 'monetization_on', label: 'Great Value', className: 'badge-great' },
  good: { icon: 'thumb_up', label: 'Good Deal', className: 'badge-good' },
  average: { icon: '', label: 'Average', className: 'badge-average' },
  poor: { icon: '', label: 'Below Avg', className: 'badge-average' },
};
...
export function CruiseCard({ cruise }: CruiseCardProps) {
  ...
  return (
    <article className="card-brutal group">
      {/* Image Section — brutalist frame */}
      <div className="relative aspect-[4/3] overflow-hidden border-hard-bottom">
        <img
          src={cruise.imageUrl}
          alt={cruise.name}
        ...
        <div className="absolute top-3 left-3">
          <span className={rating.className}>
            {rating.icon && <MaterialIcon name={rating.icon} size="sm" className="mr-[3px]" />}
            {rating.label}
          </span>
        </div>
        ...
        {cruise.priceDropPercent >= 15 && (
          <div className="absolute top-3 right-3">
            <span className="badge-price-drop">
              <MaterialIcon name="trending_down" size="xs" className="mr-[2px]" />
        ...
        {cruise.soloSupplement <= 25 && (
          <div className="absolute top-3 right-3">
            <span className="badge-solo">
              <MaterialIcon name="person" size="xs" className="mr-[2px]" />
              Solo Friendly
            </span>
          </div>
        )}
      ...
      {/* Content Section — refined data inside brutalist shell */}
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <MaterialIcon name="star" size="sm" className="text-neon-amber-400" fill />
